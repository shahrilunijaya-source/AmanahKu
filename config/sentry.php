...
return [
    ...
    // services.sentry.browser_dsn instead.
    ...
    //
    // Stays off, and not only as a default worth keeping: with PII on, the SDK attaches IP
    // addresses, cookies and headers, and the session cookie in that set is a live
    // credential for an HR account. Named here so the reason survives the next upgrade
    // that rewrites this file.
    'send_default_pii' => env('SENTRY_SEND_DEFAULT_PII', false),

    // No request body, ever. RequestIntegration would otherwise attach up to 10KB of it,
    // and on this application that body is a clock-in punch: GPS coordinates, the reason an
    // employee typed for being late, a payroll edit. That is the company's data, and it does
    // not leave the host to a third party for the sake of a stack trace that already names
    // the file and line. Hard-coded rather than env-driven — an environment variable is an
    // invitation to turn it on during a debugging session and forget.
