...
The repo of record is `developer-unijaya/claudecode/amanahku` on GitLab, which runs under URSB
governance — read the next section before making any change.
...
## Branching
`main` is protected (Maintainers only). Work lands via merge request from
`feature/<issue-id>-<slug>`; a human merges.
...
- **MCP (read-only / non-prod):** context7 (docs), playwright (verify/QA), laravel-boost
  (`.mcp.json`, local artisan only), Google Drive/Gmail/Calendar (office layer).
...
This repo stores real employee records in the app's database — **never commit a dump**
(`/storage/backups` is gitignored for exactly this reason).
