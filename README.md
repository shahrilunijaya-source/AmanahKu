...
This repo runs under the **URSB Engineering Standard**. Read `CLAUDE.md` (governance section)
before changing anything.

| Where | What |
|-------|------|
| `PROJECT.md` / `STATE.md` / `REQUIREMENTS.md` | GSD project context — owner, current phase, requirements |
| `.planning/` | Phase plans and clarification records |
| `DECISIONS/` | Architecture decisions and change proposals |
| `security/` | Security baseline for this project |
| `docs/intake/approved-requirements.md` | The machine-readable approved requirement copy |
| `.gitlab/issue_templates/Engineering-Change.md` | Raise changes here (change classification + gates) |
| `.gitlab-ci.yml` | Includes the central `ci-templates` pipeline — do not hand-roll jobs |
| `scripts/validate-project.sh` | Check the project is still correctly wired |

`main` is protected. Work lands via merge request from `feature/<issue-id>-<slug>`, merged by a
human. No client data, dumps or credentials are ever committed.
