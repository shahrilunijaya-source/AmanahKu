...
<div style="display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid var(--hairline-soft);" x-data="{ se: false }">
    <span style="flex:1;min-width:0;font-size:12.5px;color:var(--ink);{{ $sp->is_active ? '' : 'color:var(--muted);' }}">{{ $sp->name }}@unless ($sp->is_active) <span style="color:var(--muted);font-size:11px;">(<span x-text="$store.ui.lang==='en' ? 'inactive' : 'tidak aktif'">inactive</span>)</span>@endunless</span>
