...
@once
    <style>
        .see-all-btn{display:inline-flex;align-items:center;gap:8px;height:38px;padding:0 13px;border-radius:10px;border:1px solid var(--hairline);background:#fff;color:var(--muted);text-decoration:none;font-size:12px;transition:background .14s,color .14s,border-color .14s;}
    ...
    <div style="display:flex;justify-content:flex-end;margin-bottom:14px;">
        <a href="{{ route('app.screen', $target) }}" class="see-all-btn"
           ...
           title="{{ $label }}" aria-label="{{ $label }}">
            ...
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;">
                <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
                <circle cx="9" cy="7" r="4"></circle>
                <path d="M23 21v-2a4 4 0 0 0-3-3.87"></path>
                <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
            </svg>
            <span x-text="$store.ui.lang==='en' ? @js($label) : @js($labelMs ?? $label)">{{ $label }}</span>
        </a>
    </div>
@endif
