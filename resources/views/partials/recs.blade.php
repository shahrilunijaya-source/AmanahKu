...
@forelse ($recs as $r)
    <form method="POST" action="{{ route('workforce.apply') }}" style="padding:12px 0;border-bottom:1px solid #322f29;margin:0;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;">
            <span style="font-size:11px;color:var(--sidebar-dim);">{{ $r['impact'] }}</span>
...
@empty
    <div style="padding:14px 0;font-size:12.5px;color:var(--sidebar-dim);line-height:1.5;" x-text="$store.ui.lang==='en' ? 'All clear — nothing needs action right now.' : 'Semua baik — tiada tindakan diperlukan buat masa ini.'">All clear — nothing needs action right now.</div>
@endforelse
