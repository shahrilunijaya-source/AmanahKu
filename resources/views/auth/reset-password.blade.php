...
<head>
    ...
    <title>Set a new password · Amanahku</title>
...
<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:var(--canvas);padding:48px;">
    <form action="{{ route('password.update') }}" method="post" style="width:100%;max-width:380px;">
        @csrf
        ...
        <h1 style="font-weight:400;font-size:28px;letter-spacing:-0.5px;color:var(--ink);margin:0 0 8px;">Choose a new password</h1>
        <p style="font-size:14px;color:var(--muted);margin:0 0 28px;">Pick a strong password you don't use elsewhere.</p>
        ...
        <input name="email" type="email" value="{{ old('email', $request->email) }}" autocomplete="username" required style="{{ $fs }}" />

        <label style="display:block;font-size:13px;font-weight:500;color:var(--ink);margin-bottom:6px;">New password</label>
        <input name="password" type="password" autocomplete="new-password" required autofocus style="{{ $fs }}" />
        ...
        <button type="submit" class="uj-btn-primary" style="width:100%;height:46px;font-size:14px;margin-top:2px;">Reset password</button>
