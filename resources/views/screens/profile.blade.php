            ...
            <div x-show="edit" x-cloak @click.self="edit = false"
                ...
                <form method="post" action="{{ route('employees.update', $p) }}"
                    ...
                    <div style="padding:20px;overflow-y:auto;flex:1;min-height:0;display:flex;flex-direction:column;gap:10px;">
                        ...
                        <div><label style="display:block;font-size:11.5px;color:var(--muted);margin-bottom:4px;"><span x-text="$store.ui.lang==='en' ? 'Staff ID' : 'ID Staf'">Staff ID</span></label><input name="staff_id" type="text" value="{{ old('staff_id', $p->staff_id) }}" placeholder="Auto if blank" style="{{ $fs }}font-family:var(--font-mono);" /></div>
