...
<div class="uj-pt-wrap"
    ...
    @if ($canOptOut)
        <div class="uj-pt-optout">
            <span>This week's question names you: "{{ $poll->question }}". You can opt out before it opens Monday.</span>
            <form method="POST" action="{{ route('plot-twist.opt-out', $poll->id) }}">
                @csrf
    ...
    @if ($canPublish)
        <div class="uj-card" style="padding:18px 20px;display:flex;flex-direction:column;gap:10px;">
            ...
            <form method="POST" action="{{ route('plot-twist.store') }}" style="display:flex;flex-direction:column;gap:10px;">
                ...
                <label>Named person (Who only) <input type="number" name="named_employee_id"></label>
