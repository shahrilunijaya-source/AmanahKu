                            ...
                            <div x-data="{ ftAdding: false, ftEditing: null, ftEnding: null }" style="margin-top:14px;">
                                ...
                                <div x-show="ftAdding" x-cloak style="border:1px dashed var(--hairline);border-radius:8px;padding:10px;margin-top:6px;">
                                    <form method="post" action="{{ route('payroll.fixed-transactions.store') }}">
                                        ...
                                        @include('partials.hint', ['en' => 'Prorate uses calendar days in the month (joiner/leaver), not the 26-day rule used for unpaid leave/overtime. Leave End blank to run until stopped — set it (or use End on an existing row) to give it a last month. Last month RM lets that final month pay a different amount from every month before it.', 'ms' => 'Prorata guna bilangan hari kalendar dalam bulan (pekerja baru/keluar), bukan peraturan 26 hari untuk cuti tanpa gaji/kerja lebih masa. Biarkan Tamat kosong untuk ia berjalan sehingga dihentikan — tetapkan (atau guna Tamat pada baris sedia ada) untuk beri bulan akhir. RM bulan akhir membolehkan bulan terakhir itu dibayar jumlah berbeza daripada setiap bulan sebelumnya.'])
                                    </form>
                                </div>
                            </div>
