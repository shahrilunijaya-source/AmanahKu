        ...
        <div>
            ...
            <section x-show="step==='personal'" x-cloak class="uj-card" style="padding:20px 22px;">
                ...
                <form method="POST" action="{{ route('welcome.personal') }}">
                    @csrf
                    <div class="wz-grid2">
                        <div><label style="{{ $lbl }}" x-text="$store.ui.lang==='en' ? 'Nickname (optional)' : 'Nama panggilan (pilihan)'">Nickname (optional)</label><input name="nickname" value="{{ old('nickname', $employee->nickname) }}" maxlength="60" style="{{ $inp }}"><div style="font-size:11.5px;color:var(--muted);margin-top:4px;" x-text="$store.ui.lang==='en' ? 'What colleagues call you, such as &quot;Hakime&quot;. Used instead of your full name.' : 'Apa rakan sekerja panggil anda, contohnya &quot;Hakime&quot;. Digunakan sebagai ganti nama penuh anda.'">What colleagues call you, such as &quot;Hakime&quot;. Used instead of your full name.</div></div>
