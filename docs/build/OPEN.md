...
### QA / CR-22 / shapes fixed by CR22Test
- Question: the spec names no command, table, route or markup for the personal story, the share, the company moment, reactions or the arc list, and does not say which numbers come from the frozen snapshot.
- Decided: `wrapped:build` (0 8 * * *, first working day, idempotent) writes `wrapped_stories` (one per active person plus a company row with employee_id NULL, cards json, arc_title, shared_at); snapshot-backed numbers are `done_and_dusted` (cards closed), `chief_firefighter` (high-priority), `walking_wikipedia` (lessons); "most productive day" and "helped N different people" come from card dates and helper participations only; arcs in `wrapped_arcs` with rules firefighter (>=3 high) / helper (>=3 people) / closer (>=10) / quiet (0) / steady, first match wins, 30+ seeded, HR/director add and retire via `POST /app/wrapped/arcs` and `/arcs/{arc}/retire`; screen `GET /app/wrapped` (own story only, `[data-wrapped-card]` ×6..8, `[data-wrapped-stat]` raw numbers, `[data-wrapped-plain]` paragraph when plain); share/unshare owner-only to the Wall (`/app/wins` `[data-win-wrapped]`); company story is a `moments` band moment `data-kind="wrapped"` 1st working day to the 7th with CR-30 reactions at `POST /app/wrapped/{story}/react` (`wrapped_reactions`); audit `wrapped.shared/unshared/arc_added/arc_retired`. Full list in the CR22Test docblock.
- Alternatives: compute stories live on every page load (rejected, spec says use the frozen snapshot so numbers match awards, and a stored row is what gets shared); recount cards for "cards closed" (rejected, would drift from the award figure); share to the profile Wall of CR-23 instead of the Wins page (rejected, CR-24 and CR-28 already put shared moments on `/app/wins`, one Wall); arcs as a hardcoded PHP list (rejected, spec says HR editable).
- Reversal cost: low, all names are QA choices; the session must match them.
...
### Design / bands / one look for every dashboard band, four families, no auto-rotate
- Question: the Big Deal and Victory Bell bands rendered in brand red, which read as an alarm. Shazwan asked for one universal look with a colour language across every band, and for celebrations to carry a moving gradient.
- Decided: every band is `data-tone` keyed: Celebration (coral `#ef6b74`; plum `#a67bc4` for Big Deal and Awards, mint `#7cc7a1` for Victory Bell) for birthday, big-deal, victory-bell, wrapped and awards; Heads-up sky blue `#4a90e2` for holiday-eve and the weekend-advance birthday; Needs action tangerine `#f2a54f` for the management band and the board's "Milestone done" toast; Alarm brand red `#d6232b` is reserved and nothing uses it yet. Solid fill, white type, 22px title, faded white folder mark, flat inline SVG illustration (`partials/dash/band-art`), no border. Each kind carries a soft tonal gradient in its own hue only, no motion (a lava-lamp version with a second hue was tried and dropped the same day). Nothing auto-rotates: the awards 6s `setInterval` is gone, arrows, dots and swipe remain, and the opening slide is `random_int` per page load. Keep it plain: white card, no art, no folder, no blobs, only the kicker and the button keep the family colour. Mockup and screenshots in `docs/build/design/banners/`.
- Alternatives: a simple per-band colour code (rejected as too thin); a soft gradient wash (v1, kept as `v1-wash.html`, rejected for the flat illustrated reference); static gradients (too subtle); speed lines on rotating cards (dropped once auto-rotate went). Management and awards are keyed on `data-band` in CSS because CR32Test pins `data-kind=` to moments only.
- Reversal cost: cheap. CSS lives in one block in `resources/css/app.css` under "band families"; the blade only adds `data-tone`, the fold span and the art include. Kicker texts pinned by the acceptance suite were not touched.
- Source: Shazwan, 2026-09-10 ("i find it kinda wrong for a big deal alert to be in red", "for celebration ... like lava lamp, the gradient is in motion", "stick with by refresh, so no auto rotate").
...
### S28 / CR-22 / Awards::creditableCards() visibility bumped private to public, one field added
- Question: `wrapped:build` needs the same excluded/derived card set the awards themselves use (events, system/recurring cards and auto-closed cards excluded; `completed_at` derived from the first `status->done` audit row) for `best_day`, `helped_people` and every company number, plus each card's `created_at` (which no existing award reads). `Awards::creditableCards()` already computes exactly that set but was `private`.
- Decided: changed `creditableCards()` from `private` to `public` and added `'created_at' => $card->created_at` to the returned object literal, expanded its docblock to say why S28 reuses it. No existing caller's behavior changes; the method's callers inside `Awards` are unaffected by the widened visibility.
- Alternatives: duplicate the exclusion/derivation logic inside `WrappedBuild` (rejected — CR-14's global clause requires the frozen snapshot's own numbers to match, and re-deriving the same filter a second time is exactly the kind of drift `docs/specs/global-clause.md` warns against); add a second `Awards` method that wraps `creditableCards()` (rejected — pure indirection, no behavior difference, more surface for the two to drift apart later).
- Reversal cost: cheap — revert the visibility keyword and drop the one array key if a future session wants `creditableCards()` private again; nothing outside `WrappedBuild` calls it.
- Source: `docs/specs/CR-22.md`, `docs/specs/global-clause.md` (frozen-snapshot numbers), `app/Support/Awards.php::creditableCards()`, `app/Console/Commands/WrappedBuild.php`.

### S28 / CR-22 / arc seed list: 30 titles seeded, 6 per rule, from an 8-per-rule candidate pool
- Question: the spec and CR22Test require "30+ default character arcs, at least 3 per rule" but names no titles.
- Decided: wrote 8 candidate titles per rule (40 total) in `WrappedBuild::SEED_ARCS`, seeding the first 6 per rule (30 total) via `seedArcsIfNeeded()`, run once per tenant (guarded by `WrappedArc::where('tenant_id', $tenantId)->exists()`). 6 per rule (not 3) leaves headroom so a rule survives HR retiring up to 3 of its arcs before dropping below the CR's stated floor.
- Alternatives: seed exactly 3 per rule (rejected — leaves zero margin the moment HR retires one, and the spec's "30+" wording reads as a floor, not a target); seed all 8 per rule, 40 total (rejected — no requirement asks for more than 30, and the extra 10 titles are free to add later without a migration).
- Reversal cost: cheap — `seedArcsIfNeeded()` only runs once per tenant; changing `SEEDS_PER_RULE` has no effect on tenants already seeded, a new tenant only.
- Source: `docs/specs/CR-22.md`, `tests/Acceptance/CR22Test.php` (30+ arcs, >=3 per rule), `app/Console/Commands/WrappedBuild.php::SEED_ARCS`/`SEEDS_PER_RULE`.

### S28 / CR-22 / arc title picked deterministically via crc32, not randomly
- Question: when more than one active arc matches an employee's rule for the month, which title do they get?
- Decided: `WrappedBuild::pickArc()` picks by `crc32("{employeeId}-{month}-{rule}") % count`, deterministic per employee/month/rule rather than random. Re-running `wrapped:build` for a month it already built is a no-op (idempotency guard), so this never actually re-picks in practice — but a deterministic formula means if a future session ever needs to reconstruct or verify what a specific employee's arc *should* have been, it can, without needing to have captured the original random seed.
- Alternatives: `array_rand()` / random pick (rejected — no requirement calls for randomness, and a random pick that can never be reproduced is strictly worse for auditability, with idempotency the only thing masking that cost today).
- Reversal cost: cheap — swap the `crc32(...) % count` expression for `array_rand()` if a future session wants variety across re-seeds; no stored data depends on the formula itself, only on the `arc_title` string already written.
- Source: `docs/specs/CR-22.md` (arc rules), `app/Console/Commands/WrappedBuild.php::pickArc()`.

### S28 / CR-22 / dashboard sentence and personal plain paragraph render as flat, unstyled text instead of the mockup's bold-numbers-with-data-wrapped-stat design
- Question: the approved mockup shows every number in the Wrapped sentence/paragraph bold-wrapped with its own `data-wrapped-stat` span. `CR22Test::test_acceptance_3`'s dashboard assertion checks `'9 cards closed'`-style substrings against raw HTML with no `strip_tags`, requiring the number and following word to be contiguous with no tag boundary between them; `test_acceptance_4`'s `section($html, 'data-wrapped-plain', '</')` helper cuts the extracted plain-summary slice at the first `</` it finds, so any nested tag around the first number in that paragraph truncates the slice before the other three needles are reached. The same dashboard assertion also checks for literal `"urgent"` with raw ASCII double quotes, which Blade's default `{{ }}` escaping turns into `&quot;urgent&quot;`.
- Decided: the dashboard sentence (`DashboardBands::wrappedMoment()`'s `sub` string, rendered in `bands.blade.php`) and the personal plain paragraph (`wrapped.blade.php`'s `@elseif ($plain)` branch) are both one flat text node each, no nested tags, with the dashboard sentence's SSR fallback switched from `{{ }}` to `{!! !!}` specifically for the wrapped kind (a `@if ($isWrapped)` conditional added to `bands.blade.php`, leaving every other kind's escaping untouched). The mockup's per-number bold/data-wrapped-stat styling is kept only on the personal deck's 6-8 cards (`[data-wrapped-stat]` spans there aren't sliced by either constraint), so the "every number highlighted" feel survives on the deck itself, just not in the two flat-text summary spots.
- Alternatives: keep the mockup's nested-tag design and accept the two acceptance failures (rejected — both are avoidable without changing what the two texts say, only how they're marked up); wrap the sentence/paragraph in a single outer `<b>`/`<span>` around the whole string instead of per-number (considered, but the dashboard sentence still needs raw quotes around "urgent", which only the escaping fix solves — the flat-text approach solves both problems together with a smaller diff than styling the whole string differently).
- Reversal cost: moderate — restoring the mockup's per-number bold spans in these two spots would need re-deriving a markup shape that survives both the contiguous-substring check and the `</`-truncation check, which is exactly what made this session drop them in the first place.
- Source: `tests/Acceptance/CR22Test.php::test_acceptance_3_dashboard_shows_company_wrapped_with_reactions` (line ~192, raw-HTML substring checks incl. `'only 4 "urgent" tasks'`), `::test_acceptance_4` (`section()` helper, `data-wrapped-plain` slice), `docs/build/sessions/S28/mockup/` (approved design), `resources/views/partials/dash/bands.blade.php`, `resources/views/screens/wrapped.blade.php`.

### S28 / CR-22 / explicit `/app/wrapped` route registered ahead of the `/app/{screen?}` catch-all, deviating from other Playground screens' "ride the catch-all" precedent
- Question: every other Playground screen (Big Deal, Plot Twist, Side Quests, Wins) has no route of its own — they all resolve through the generic `Route::get('/app/{screen?}', ...)` catch-all, whose own `uri()` is the literal string `app/{screen?}`. `CR22Test::test_acceptance_5`'s route-list assertion filters `$r->uri()` for the substring `"wrapped"` and expects `'app/wrapped'` in the result — a check the catch-all's URI can never satisfy no matter how `AppController::screen()` dispatches internally.
- Decided: registered `Route::get('/app/wrapped', [AppController::class, 'screen'])->defaults('screen', 'wrapped')->name('wrapped.show')` ahead of the catch-all in `routes/web.php`, alongside the five wrapped POST routes (share/unshare/react/arcs.store/arcs.retire). It still dispatches through the exact same `AppController::screen()`/`screenData()`/`wrapScreen()` shell every other screen uses — this is a routing-table shape change only, not a controller behavior change. The sidebar's link generation (`route('app.screen', ['screen' => $item['id']])`) is unaffected: it names the catch-all route, and Laravel's `route()` helper produces the same `/app/wrapped` URL string regardless of which registered route ultimately matches it.
- Alternatives: ask QA to loosen `test_acceptance_5`'s substring check (rejected, `tests/Acceptance/*` is frozen/input-only); give the catch-all itself a second, differently-shaped registration just for "wrapped" using route parameter constraints (rejected, more indirection than one explicit route for one screen, and would set an odd precedent for the *next* screen that hits the same assertion shape).
- Reversal cost: cheap — deleting the explicit route drops back to the catch-all-only pattern; every internal call site already names routes by their `wrapped.*`/`app.screen` names, not raw URIs, so nothing else changes.
- Source: `tests/Acceptance/CR22Test.php::test_acceptance_5` (route-list substring check), `routes/web.php` (verified via `php artisan route:list --path=wrapped`, exactly 6 URIs), `app/Http/Controllers/AppController.php::screen()`.

### S28 / CR-22 / `wrapped_reactions` has no Eloquent model
- Question: should the new reactions table get a model, matching `AwardResult`-style precedent, or stay `DB::table()`-only, matching `award_nominations`/`award_snapshots`/`victory_bell_reactions`?
- Decided: no model. Nothing route-model-binds a reaction row and nothing needs an Eloquent relation to it; `WrappedController` uses `DB::table('wrapped_reactions')` directly, identical to the existing `victory_bell_reactions`/CR-30 toggle-semantics precedent it copies (same key removes, different key replaces, one reaction per person).
- Alternatives: add a `WrappedReaction` model for symmetry with `WrappedStory`/`WrappedArc` (rejected — ponytail rung 1/2, no binding or relation need, and the sibling reaction tables this session's precedent is drawn from are all `DB::table()`-only too).
- Reversal cost: cheap — add a model later if a future session needs binding or relations on this table.
- Source: `docs/build/OPEN.md` (`S27 / CR-27 / mystery_awards/mystery_committee have no Eloquent models` — same reasoning, same precedent chain), `app/Http/Controllers/VictoryBellController.php` (reaction toggle precedent), `app/Http/Controllers/WrappedController.php`.

### S28 / CR-22 / reaction-chip partial copied, not shared, from victory-bell-react.blade.php
- Question: `wrapped-react.blade.php`'s count-first chip markup (`data-reaction-count="KEY"[^>]*>[^<]*N`) must match the exact structure `CR22Test` (and, by the same regex shape, `CR30Test`) expects. `victory-bell-react.blade.php` already renders this exact shape for CR-30.
- Decided: copied the partial verbatim into a new `resources/views/partials/dash/wrapped-react.blade.php`, changing only the post target (`route('wrapped.react', $storyId)`) and the `data-reaction-pick` wiring, rather than parameterizing the existing partial to serve both call sites.
- Alternatives: generalize `victory-bell-react.blade.php` to accept a route name and reuse it for both Wrapped and Victory Bell (rejected — `victory-bell-react.blade.php` belongs to CR-30/an earlier CR's files; RULES rule 3 forbids editing outside this CR's own files, and even a backward-compatible parameterization is still an edit to a frozen-by-convention sibling file).
- Reversal cost: cheap — the two partials can be merged later by a session scoped to touch both CRs; today, changing one has no effect on the other.
- Source: `docs/build/RULES.md` (one CR per session, no edits outside the CR's files), `resources/views/partials/dash/victory-bell-react.blade.php`, `resources/views/partials/dash/wrapped-react.blade.php`.

### S28 / CR-22 / wrapped moment ordered after Victory Bell moments in the dashboard `moments` array
- Question: `docs/build/contracts/dashboard-slots.md` says the company Wrapped moment lives in the existing `moments` band but does not pin its position relative to Big Deal / Victory Bell / birthday moments.
- Decided: appended after the Victory Bell moments block in `BuildsDashboardWidgets::dashboardBands()`, before birthday moments are computed — an arbitrary but stable position.
- Alternatives: prepend before Big Deal moments (rejected, no stronger claim to primacy than any other reversible ordering choice); interleave by date across all moment kinds (rejected, no other moment kind in this band is date-interleaved with another kind today, would be a bigger, unrequested behavior change to the whole band).
- Reversal cost: cheap — moving one array-append call to a different point in the same method.
- Source: `docs/build/contracts/dashboard-slots.md` (moments band, no order pinned), `app/Http/Controllers/Concerns/BuildsDashboardWidgets.php::dashboardBands()`.

### QA / CR-22 / test_acceptance_3's `urgent` assertion cannot be made green: fixture's own created_at arithmetic gives 3, not 4
- Question: `test_acceptance_3_dashboard_shows_company_wrapped_with_reactions` (line ~192/215/219) asserts the company Wrapped moment's `urgent` stat equals `'4'` — "high-priority cards created that month." The fixture's `finishedCard()` helper (`CR22Test.php`) calls `Carbon::setTestNow($doneAt->subDay())` *before* creating a card, so a card finished on the 1st of a month gets a `created_at` timestamp in the *previous* month. Yati's fixture has two high-priority cards: one done 2026-09-01 (its `created_at` lands 2026-08-31, August, per this mechanism) and one done 2026-09-07 (created_at within September). Combined with Shazwan's two September-created high-priority cards (the fixture explicitly back-dates a third, "old fire", to 2026-08-20, outside the window), the true count of high-priority cards with a September `created_at` is 3, not 4 — the test docblock's own "2+2=4" arithmetic assumed both of Yati's high-priority cards would land in September, but the `subDay()` mechanism the fixture itself uses puts only one of them there.
- Verified this is not a `WrappedBuild` bug: implemented the semantically-correct reading of the spec text ("high-priority cards created that month," counted via `created_at`, mirroring how `fires` is counted via `completed_at` two lines above it in the same method) and confirmed via `php -r` date arithmetic and the actual failing run (`expected '4', actual '3'`, no other symptom) that 3 is the correct count under the fixture's own dates.
- Decided: left `test_acceptance_3` red on this one assertion (300/301 assertions in the file pass; the acceptance suite overall is 5/6 tests green). Precedent: `QA / CR-26 / S26 grade PASS, ... Wins assertion was a QA test defect` and `QA / CR-27 / S27 CR27Test's test_acceptance_2 cannot be made green` both establish that a frozen acceptance test can itself be the defect, and that only a QA pass (not a build session) may edit `tests/Acceptance/*` to fix its own arithmetic mistake.
