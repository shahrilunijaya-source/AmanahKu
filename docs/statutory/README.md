...
| PERKESO SOCSO + EIS + SKBBK | `PerkesoBorang8A` | `perkeso-8a-2026-06.txt` | PERKESO "Combine SOCSO + EIS Contribution Text File Format" v2.1, 13 Feb 2026 (see below) | true |
...
audit trail records the download as an unverified layout.

### PERKESO contribution file (ASSIST 2.0)

From 1 October 2026 PERKESO accepts only its combined text file (`perkeso-faq-text-file-format-en.pdf`).
The spec itself, `perkeso-text-file-format-v2.1-2026-02.pdf`, is marked confidential and not for
redistribution, so it is gitignored; ask for a copy or download it from PERKESO. In short: fixed
width, 278 characters a line, CRLF between lines:

| Positions | Field |
|---|---|
| 1-12 | employer code, left justified |
| 13-32 | MyCoID / SSM number, optional |
| 33-44 | NRIC, or SSFW / SSFDW number for a foreign worker |
| 45-194 | name, left justified |
| 195-200 | contribution month `MMYYYY`, the month the wages were earned |
| 201-214 | actual wages in cents (not capped) |
| 215-220, 221-226 | SOCSO employer, employee, in cents |
| 227-232, 233-238 | EIS employer, employee, in cents |
| 239-244 | SKBBK employee, in cents, `0000` when none |
| 245-278 | blank |

Amounts are right justified with spaces, and a zero amount is written `0000`, following the sample
file on the spec's last page. The PERKESO FAQ says to add SKBBK into the SOCSO employee share, but
the spec gives it its own field 11 and the sample fills it, so the spec wins.
