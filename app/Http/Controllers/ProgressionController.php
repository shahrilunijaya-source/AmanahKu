...
class ProgressionController extends EmploymentRecordController
{
    ...
    public function updateRecord(Request $request, EmployeeProgression $progression): RedirectResponse
    {
        ...
        $lastWorkingDay = null;
        if ($progression->type === 'resigned' && $data['last_working_day']) {
            $lastWorkingDay = CarbonImmutable::parse($data['last_working_day']);
            if ($lastWorkingDay->lt($on)) {
        ...
        $progression->forceFill(['effective_on' => $on->toDateString(), 'remark' => $data['remark'] ?: null, 'snapshot' => $snapshot])->save();
    ...
    /** Mirrors a corrected resigned row's last working day onto the employee, only when this is their latest resignation. */
